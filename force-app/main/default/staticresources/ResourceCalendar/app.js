...
(function() {
  ...
  let appointmentSource = {
    ...
    eventDataTransform: function(record) {
      if (viewModel.selectedZone && viewModel.selectedZone !== 'All') {
        let zoneResourceIds = viewModel.getZoneResourceIdSet();
        if (!record.ServiceResources || record.ServiceResources.length === 0) {
          return false;
        }
        let matchesZone = record.ServiceResources.some(v => zoneResourceIds.has(v.Id));
        if (!matchesZone) {
          return false;
        }
      }
      if (viewModel.showAbsencesOnly) {
        return false;
      }
  ...
  let absenceSource = {
    ...
    display: 'block',
    events: function(info, onSuccess, onFailure) {
      let resourceIds = viewModel.getAbsenceResourceIds();
  ...
  function ViewModel() {
    ...
    this.showAll = function() {
      ...
      self.ownerId = '';
      self.showAbsencesOnly = false;
      self.selectedZone = 'All';
      CALENDAR.refetchEvents();
      bind.update(self);
    ...
    this.toggleAbsencesOnly = function() {
      self.showAbsencesOnly = !self.showAbsencesOnly;
      CALENDAR.refetchEvents();
      bind.update(self);
    };

    this.onZoneChange = function() {
      CALENDAR.refetchEvents();
      bind.update(self);
    };

    this.getZoneResourceIdSet = function() {
      let set = new Set();
      if (!self.selectedZone || self.selectedZone === 'All') {
        return set;
      }
      self.resources.forEach(r => {
        if (r.Territories && r.Territories.includes(self.selectedZone)) {
          set.add(r.Id);
        }
      });
      return set;
    ...
    this.getAbsenceResourceIds = function() {
      if (self.selectedResources.length > 0) {
        return self.selectedResources;
      }
      if (self.selectedZone && self.selectedZone !== 'All') {
        return Array.from(self.getZoneResourceIdSet());
      }
      return [];
    };
    ...
    ResourceCalendarController.getResources((result, event) => {
      if (event.status) {
        ...
        self.resourceById = result.reduce((a, c) => {
          a[c.Id] = c;
          return a;
        }, {});
        let zoneSet = new Set();
        result.forEach(r => {
          if (r.Territories && r.Territories.length > 0) {
            r.Territories.forEach(t => zoneSet.add(t));
          }
        });
        self.zones = ['All'].concat(Array.from(zoneSet).sort());
      } else {
  ...
  rivets.formatters.defaultPhoto = function(value) {
    return value ? value : defaultPhotoUrl;
  };

  rivets.formatters.matchesZone = function(resource, selectedZone) {
    if (!selectedZone || selectedZone === 'All') {
      return true;
    }
    if (!resource || !resource.Territories) {
      return false;
    }
    return resource.Territories.includes(selectedZone);
  };
